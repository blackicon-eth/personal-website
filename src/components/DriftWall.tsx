...
export function DriftWall({
  ...
  const maskStyle =
    "linear-gradient(to right, transparent, #000 calc((100% - var(--dw-edge)) * 0.6), #000 calc(100% - (100% - var(--dw-edge)) * 0.6), transparent), " +
    "linear-gradient(to bottom, transparent, #000 calc(100% - var(--dw-edge)), #000 var(--dw-edge), transparent)";
  ...
  const imgClass = cx(
    "block max-h-full max-w-full select-none rounded-xl object-contain",
  ...
  return (
    ...
    >
      <div
        ref={planeRef}
        className="absolute left-[55%] top-1/2 flex cursor-pointer flex-row transform-3d origin-[50%_50%] will-change-transform"
      >
